...
!
  ...
  REAL :: t (nx,ny,nz)  ! Perturbation potential temperature (K)
  REAL :: p (nx,ny,nz)  ! Perturbation pressure (Pascal)

  REAL :: qv    (nx,ny,nz)  ! Water vapor specific humidity (kg/kg)

  REAL, TARGET :: qscalar(nx,ny,nz,nscalar)
...
!-----------------------------------------------------------------------
  ...
  DO k=1,nz-1
    DO j=1,ny-1
      DO i=1,nx-1
        qv(i,j,k)=MAX(0.0,qv(i,j,k))
        qc(i,j,k)=MAX(0.0,qc(i,j,k))
        qr(i,j,k)=MAX(0.0,qr(i,j,k))
        qi(i,j,k)=MAX(0.0,qi(i,j,k))
        qs(i,j,k)=MAX(0.0,qs(i,j,k))
        qh(i,j,k)=MAX(0.0,qh(i,j,k))
...
!    END IF
    ...
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          t(i,j,k)=(t(i,j,k)+ptbar(i,j,k))*ppi(i,j,k)
          p(i,j,k)=(p(i,j,k)+pbar(i,j,k))
        END DO
      END DO
    END DO
!
...
!
    ...
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          rliq(i,j,k) = qc(i,j,k) + qr(i,j,k)
          rice(i,j,k) = qi(i,j,k) + qs(i,j,k) + qh(i,j,k)
        END DO
      END DO
    END DO

! rtot0 = qv(i,j,k) + rliq(i,j,k) + rice(i,j,k)
...
! write (*,*) "ZUWEN subsatopt/rhsat", subsatopt, rhsat 
    ...
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          rsat(i,j,k) = rhsat*f_qvsatl( p(i,j,k), t(i,j,k) )

          IF ( .NOT. (rliq(i,j,k) == 0. .AND.                             &
                      qv(i,j,k) < rsat(i,j,k)) ) THEN

! Call SatAdjL (qv(i,j,k), p(i,j,k), t(i,j,k), rsat(i,j,k), t1)

            t1(i,j,k) = t(i,j,k)
            rsat(i,j,k) = rhsat*f_qvsatl( p(i,j,k), t1(i,j,k) )
            dr = qv(i,j,k) - rsat(i,j,k)
            err_num = t1(i,j,k) - t(i,j,k) - lvl/cp * dr
            ...
            qc(i,j,k) = qc(i,j,k) + (qv(i,j,k)-rsat(i,j,k))
            ...
            ELSE ! evaporation of cloud liquid, then rain
              ...
              qv(i,j,k) = rsat(i,j,k) + qc(i,j,k)
              qc(i,j,k) = 0.
  
              IF (qr(i,j,k) > 0. .AND. qv(i,j,k) < rsat(i,j,k)) THEN
                rate(i,j,k) = r2v * (rsat(i,j,k)-qv(i,j,k))
                change(i,j,k) = rate(i,j,k) * delt
                IF (change(i,j,k) > (rsat(i,j,k)-qv(i,j,k)))         &
                    change(i,j,k)=(rsat(i,j,k)-qv(i,j,k))
                IF (change(i,j,k) > qr(i,j,k)) THEN
                  qv(i,j,k) = qv(i,j,k) + qr(i,j,k)
                  qr(i,j,k) = 0.
                ELSE
                  qv(i,j,k) = qv(i,j,k) + change(i,j,k)
                  qr(i,j,k) = qr(i,j,k) - change(i,j,k)
                END IF
              END IF
  
              x(i,j,k) = qr(i,j,k) - rliq(i,j,k)
              t(i,j,k) = t(i,j,k) + x(i,j,k)*lvl/cp
...
!
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          rsat(i,j,k) = rhsat*f_qvsati( p(i,j,k), t(i,j,k) )
          IF (.NOT.(rice(i,j,k) == 0. .AND. qv(i,j,k) < rsat(i,j,k))) THEN
  
  ! Call SatAdjI (qv(i,j,k), p, t, rsat(i,j,k), t1(i,j,k))
  
            t1(i,j,k) = t(i,j,k)
            rsat(i,j,k) = rhsat*f_qvsati( p(i,j,k), t1(i,j,k) )
            dr = qv(i,j,k) - rsat(i,j,k)
            err_num = t1(i,j,k) - t(i,j,k) - lvi/cp * dr
            err_den = 1. + lvl/cp * rsat(i,j,k)      & ! 1+Lv1/cp*d(qvs)/dt
                     * (rddrv+(1.-rddrv)*rsat(i,j,k))                     &
                         * f_desdti(t1(i,j,k))
            ...
            IF (qv(i,j,k) > rsat(i,j,k)) THEN ! ice growth
  
              IF (qc(i,j,k) > 0. .AND. qi(i,j,k) < 1E-6) THEN
  
  !    Call Nucleate_pristine (qv(i,j,k), rsat(i,j,k),
  !  :       t(i,j,k), rho(i,j,k), rp_nuc(i,j,k))
  
                rp_nuc(i,j,k) = 0.
                IF (.NOT.(t(i,j,k) > 268.)) THEN
  
                  IF (rp_nuc(i,j,k) > qi(i,j,k)) THEN
                    qv(i,j,k) = qv(i,j,k) + qi(i,j,k)
                    qi(i,j,k) = rp_nuc(i,j,k)
                    qv(i,j,k) = qv(i,j,k) - rp_nuc(i,j,k)
                  END IF
  
                ELSE
  
                  rp_nuc(i,j,k) = 1E3 * EXP(-.639+12.96*(qv(i,j,k)   &
                                            / rsat(i,j,k)-1.))            &
                                * pmas / rho(i,j,k)
  
                  IF (rp_nuc(i,j,k) > (qv(i,j,k)-rsat(i,j,k))/2.)    &
                      rp_nuc(i,j,k) = (qv(i,j,k)-rsat(i,j,k))/2.
              ...
              IF (qi(i,j,k) > 0. .AND. qv(i,j,k) > rsat(i,j,k)) THEN
                rate(i,j,k) = v2p*(qv(i,j,k)-rsat(i,j,k)) *          &
                              (qi(i,j,k)*rho(i,j,k))
                change(i,j,k) = rate(i,j,k) * delt
  
                IF (change(i,j,k) > (qv(i,j,k)-rsat(i,j,k)))         &
                    change(i,j,k)=(qv(i,j,k)-rsat(i,j,k))
  
                qi(i,j,k) = qi(i,j,k) + change(i,j,k)
                qv(i,j,k) = qv(i,j,k) - change(i,j,k)
              END IF
...
!
              IF (qi(i,j,k) > 0. .AND. qv(i,j,k) < rsat(i,j,k)) THEN
                ...
                rate(i,j,k) = p2v * (rsat(i,j,k)-qv(i,j,k))
                change(i,j,k) = rate(i,j,k) * delt
  
                IF (change(i,j,k) > (rsat(i,j,k)-qv(i,j,k)))         &
                    change(i,j,k)=(rsat(i,j,k)-qv(i,j,k))
  
                IF (change(i,j,k) > qi(i,j,k)) THEN
                  qv(i,j,k) = qv(i,j,k) + qi(i,j,k)
                  qi(i,j,k) = 0.
                ELSE
                  qv(i,j,k) = qv(i,j,k) + change(i,j,k)
...
!-----------------------------------------------------------------------
!
              IF (qs(i,j,k) > 0. .AND. qv(i,j,k) < rsat(i,j,k)) THEN
  
                rate(i,j,k) = s2v * (rsat(i,j,k)-qv(i,j,k))
                change(i,j,k) = rate(i,j,k) * delt
  
                IF (change(i,j,k) > (rsat(i,j,k)-qv(i,j,k)))         &
                    change(i,j,k)=(rsat(i,j,k)-qv(i,j,k))
  
                IF (change(i,j,k) > qs(i,j,k)) THEN
                  qv(i,j,k) = qv(i,j,k) + qs(i,j,k)
                  qs(i,j,k) = 0.
                ELSE
                  qv(i,j,k) = qv(i,j,k) + change(i,j,k)
                  qs(i,j,k) = qs(i,j,k) - change(i,j,k)
                END IF
              END IF
...
!-----------------------------------------------------------------------
!
              IF (qh(i,j,k) > 0. .AND. qv(i,j,k) < rsat(i,j,k)) THEN
  
                rate(i,j,k) = i2v * (rsat(i,j,k)-qv(i,j,k))
                change(i,j,k) = rate(i,j,k) * delt
  
                IF (change(i,j,k) > (rsat(i,j,k)-qv(i,j,k)))         &
                    change(i,j,k)=(rsat(i,j,k)-qv(i,j,k))
  
                IF (change(i,j,k) > qh(i,j,k)) THEN
                  qv(i,j,k) = qv(i,j,k) + qh(i,j,k)
                  qh(i,j,k) = 0.
                ELSE
                  qv(i,j,k) = qv(i,j,k) + change(i,j,k)
                  qh(i,j,k) = qh(i,j,k) - change(i,j,k)
                END IF
              END IF
  
            END IF
  
            x(i,j,k) = (qi(i,j,k) + qs(i,j,k) +                 &
                       qh(i,j,k)) - rice(i,j,k)
            t(i,j,k) = t(i,j,k) + x(i,j,k)*lvi/cp
            rice(i,j,k) = qi(i,j,k) +                                &
                          qs(i,j,k) + qh(i,j,k)
    ...
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          ...
          IF (t(i,j,k) > 273.1) THEN
  
            maxmelt(i,j,k) = (t(i,j,k)-273.1) * cp/lli
  
            IF (qi(i,j,k) > 0.) THEN
              IF (maxmelt(i,j,k) < qi(i,j,k)) THEN
                qc(i,j,k) = qc(i,j,k) + maxmelt(i,j,k)
                qi(i,j,k) = qi(i,j,k) - maxmelt(i,j,k)
                GO TO 501
              ELSE
                maxmelt(i,j,k) = maxmelt(i,j,k) - qi(i,j,k)
            ...
            IF (qs(i,j,k) > 0.) THEN
              IF (maxmelt(i,j,k) < qs(i,j,k)) THEN
                qr(i,j,k) = qr(i,j,k) + maxmelt(i,j,k)
                qs(i,j,k) = qs(i,j,k) - maxmelt(i,j,k)
                GO TO 501
              ELSE
                maxmelt(i,j,k) = maxmelt(i,j,k) - qs(i,j,k)
                qr(i,j,k) = qr(i,j,k) + qs(i,j,k)
                qs(i,j,k) = 0.
              END IF
            END IF
  
            IF (qh(i,j,k) > 0.) THEN
              rate(i,j,k) = i2r * (t(i,j,k)-273.1)/5.
              change(i,j,k) = rate(i,j,k)*delt
...
!-----------------------------------------------------------------------
!
            IF (qc(i,j,k) > 0. .AND. t(i,j,k) < 253.) THEN
              rate(i,j,k) = c2p * ((253.-t(i,j,k))/20.)**2
              change(i,j,k) = rate(i,j,k) * delt
              IF (change(i,j,k) > qc(i,j,k)) THEN
                qi(i,j,k) = qi(i,j,k) + qc(i,j,k)
                qc(i,j,k) = 0.
              ELSE
                qi(i,j,k) = qi(i,j,k) + change(i,j,k)
                qc(i,j,k) = qc(i,j,k) - change(i,j,k)
              END IF
            END IF
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
            IF (qr(i,j,k) > 0. .AND. t(i,j,k) < 267.) THEN
              rate(i,j,k) = r2i * ((267.-t(i,j,k))/14.)**2
              change(i,j,k) = rate(i,j,k) * delt
              IF (change(i,j,k) > qr(i,j,k)) THEN
                qh(i,j,k) = qh(i,j,k) + qr(i,j,k)
                qr(i,j,k) = 0.
              ELSE
                qh(i,j,k) = qh(i,j,k) + change(i,j,k)
                qr(i,j,k) = qr(i,j,k) - change(i,j,k)
...
!  x = (qi(i,j,k) + qs(i,j,k) + qh(i,j,k)) -
!    :     rice(i,j,k)
!  t = t + x*Lli/cp
!
!-----------------------------------------------------------------------
!
          x(i,j,k) = (qc(i,j,k) + qr(i,j,k)) - rliq(i,j,k)
          t(i,j,k) = t(i,j,k) - x(i,j,k)*lli/cp
...
!
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          qc(i,j,k) = qc(i,j,k) * rho(i,j,k)
          qi(i,j,k) = qi(i,j,k) * rho(i,j,k)
          qr(i,j,k) = qr(i,j,k) * rho(i,j,k)
          qs(i,j,k) = qs(i,j,k) * rho(i,j,k)
          qh(i,j,k) = qh(i,j,k) * rho(i,j,k)
        END DO
      END DO
    END DO
...
!
      ...
      DO j=1,ny-1
        DO i=1,nx-1
          IF (qc(i,j,k) > qcmin) THEN
            change(i,j,k) = qc(i,j,k) - qcmin
            IF (change(i,j,k) > qr(i,j,k)) THEN
              change(i,j,k) = change(i,j,k) - qr(i,j,k)
              qr(i,j,k) = qr(i,j,k) + change(i,j,k)
              qc(i,j,k) = qc(i,j,k) - change(i,j,k)
            END IF
          END IF
    ...
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          IF (qi(i,j,k) > qpmin) THEN
            change(i,j,k) = qi(i,j,k) - qpmin
            IF (change(i,j,k) > qs(i,j,k)) THEN
              change(i,j,k) = change(i,j,k) - qs(i,j,k)
              qs(i,j,k) = qs(i,j,k) + change(i,j,k)
              qi(i,j,k) = qi(i,j,k) - change(i,j,k)
            END IF
          END IF
...
!
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          IF (qc(i,j,k) > 0.) THEN
            rate(i,j,k) = c2r * qc(i,j,k) * qr(i,j,k)
            needr(i,j,k) = rate(i,j,k)*delt
            rate(i,j,k) = c2s * qc(i,j,k) * qs(i,j,k)
            needs(i,j,k) = rate(i,j,k)*delt
            rate(i,j,k) = c2i * qc(i,j,k) * qh(i,j,k)
            needi(i,j,k) = rate(i,j,k)*delt
            tem15(i,j,k) = needr(i,j,k) + needs(i,j,k) + needi(i,j,k)
            IF (tem15(i,j,k) > qc(i,j,k)) THEN
              needr(i,j,k) = needr(i,j,k) * qc(i,j,k) / tem15(i,j,k)
              needs(i,j,k) = needs(i,j,k) * qc(i,j,k) / tem15(i,j,k)
              needi(i,j,k) = needi(i,j,k) * qc(i,j,k) / tem15(i,j,k)
            END IF
...
!-----------------------------------------------------------------------
!
            qi(i,j,k) = qi(i,j,k) + .01*needs(i,j,k)
            needs(i,j,k) = needs(i,j,k) - .01*needs(i,j,k)
            qi(i,j,k) = qi(i,j,k) + .01*needi(i,j,k)
...
!-----------------------------------------------------------------------
!
            qr(i,j,k) = qr(i,j,k) + needr(i,j,k)
            qh(i,j,k) = qh(i,j,k) + needi(i,j,k) + needs(i,j,k)
            qc(i,j,k) = qc(i,j,k) - needr(i,j,k) -              &
...
!
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          IF (qi(i,j,k) > 0. .AND. qs(i,j,k) > 0.) THEN
            eff = 1. - (273.1-t(i,j,k))/50.
            rate(i,j,k) = p2s * eff * qi(i,j,k) * qs(i,j,k)
            change(i,j,k) = rate(i,j,k) * delt
            IF (change(i,j,k) > qi(i,j,k)) THEN
              qs(i,j,k) = qs(i,j,k) + qi(i,j,k) - 1E-6
              qi(i,j,k) = 1E-6
            ELSE
              qs(i,j,k) = qs(i,j,k) + change(i,j,k)
              qi(i,j,k) = qi(i,j,k) - change(i,j,k)
    ...
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          ...
          IF (.NOT.(qc(i,j,k) == 0. .AND. qv(i,j,k) < rsat(i,j,k))) THEN
  
  ! Call SatAdjL (qv(i,j,k), p, t, rsat(i,j,k), t1)
    ...
    DO k=1,nz-1
      DO j=1,ny-1
        DO i=1,nx-1
          t(i,j,k)=(t(i,j,k)/ppi(i,j,k))-ptbar(i,j,k)
          p(i,j,k)=p(i,j,k)-pbar(i,j,k)
          qv(i,j,k)=MAX(0.0,qv(i,j,k))
          qc(i,j,k)=MAX(0.0,qc(i,j,k))
          qr(i,j,k)=MAX(0.0,qr(i,j,k))
...
!
  DO k=1,nz-1
    DO j=1,ny-1
      DO i=1,nx-1
        tvq(i,j,k)=0.0
        IF (qi(i,j,k) > 0.) tvq(i,j,k) = 0.5 * SQRT(1./rho(i,j,k))
      END DO
    END DO
  END DO
  ...
  DO k=1,nz-1
    DO j=1,ny-1
      DO i=1,nx-1
        tvq(i,j,k)=0.0
        IF (qr(i,j,k) > 0.) tvq(i,j,k) =                           &
            5.5 * (1000.*qr(i,j,k))**.125 * SQRT(1./rho(i,j,k))
      END DO
    END DO
  END DO
  ...
  DO k=1,nz-1
    DO j=1,ny-1
      DO i=1,nx-1
        tvq(i,j,k)=0.0
        IF (qs(i,j,k) > 0.) tvq(i,j,k) =                           &
            2.0 * (1000.*qs(i,j,k))**.100 * SQRT(1./rho(i,j,k))
      END DO
    END DO
  END DO
  ...
  DO k=1,nz-1
    DO j=1,ny-1
      DO i=1,nx-1
        tvq(i,j,k)=0.0
        IF (qh(i,j,k) > 0.) tvq(i,j,k) = 5.5 *                     &
            (1000.*qh(i,j,k))**.333 * SQRT(1./rho(i,j,k))
      END DO
    END DO
  END DO
