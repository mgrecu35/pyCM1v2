...
!!!                ,isftcflx,iz0tlnd,scm_force_flux,                               &
                ...
                CD(ims,j),CDA(ims,j),tst(ims,j),qst(ims,j),psiq(ims,j)  &
!!!#endif
                ,z0t(ims,j),z0q(ims,j)                             &
                                                                   )
      ENDDO
...
!!!                     isftcflx, iz0tlnd,scm_force_flux,                            &
                     isftcflx, iz0tlnd,                                           &
                     ustm,ck,cka,cd,cda,tst,qst,psiqout,z0tout,z0qout)
!-------------------------------------------------------------------
      IMPLICIT NONE
!-------------------------------------------------------------------
...
!  for land points QSFC can come from previous time step
        if(xland(i).gt.1.5.or.qsfc(i).le.2.0e-10)QSFC(I)=EP2*E1/(PSFC(I)-E1)                                                 
