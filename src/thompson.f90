...
!..Adapted for CM1: 13 June 2018
!..Updated to WRFV4.3 code:  11 June 2021
!+---+-----------------------------------------------------------------+
...
!!!      REAL, PARAMETER, PRIVATE:: Nt_c = 250.E6
    ! 220313: made Nt_c a CM1 namelist parameter (set in thompson_init)
      REAL :: Nt_c
...
!ctrlL
      ...
      INTEGER, INTENT(IN):: ids,ide, jds,jde, kds,kde, &
                            ims,ime, jms,jme, kms,kme, &
                            its,ite, jts,jte, kts,kte
      REAL, DIMENSION(ims:ime,jms:jme,kms:kme), INTENT(IN):: hgt
      logical, intent(in) :: dowr
      real, intent(in) :: nt_c_in

!..OPTIONAL variables that control application of aerosol-aware scheme

      REAL, DIMENSION(ims:ime,jms:jme,kms:kme), OPTIONAL, INTENT(INOUT) :: nwfa, nifa
      REAL, DIMENSION(ims:ime,jms:jme), OPTIONAL, INTENT(INOUT) :: nwfa2d
      REAL, DIMENSION(ims:ime,jms:jme,kms:kme), OPTIONAL, INTENT(IN) :: orho
      ...
      do k = kts, kte
         write(mp_debug,*) ' DEBUGT  k, hgt = ', k, hgt(its+1,jts+1,k)
         CALL wrf_debug(250, mp_debug)
      enddo
...
!.. fill in just a basic vertical profile, somewhat boundary-layer following.
      ...
      if (max_test .lt. eps) then
         ...
         do i = its, min(ide-1,ite)
            if (hgt(i,j,1).le.1000.0) then
               h_01 = 0.8
            elseif (hgt(i,j,1).ge.2500.0) then
               h_01 = 0.01
            else
               h_01 = 0.8*cos(hgt(i,j,1)*0.001 - 1.0)
            endif
            niCCN3 = -1.0*ALOG(naCCN1/naCCN0)/h_01
            nwfa(i,j,1) = naCCN1+naCCN0*exp(-((hgt(i,j,2)-hgt(i,j,1))/1000.)*niCCN3)
            airmass = 1./orho(i,j,1) * (hgt(i,j,2)-hgt(i,j,1))*dx*dy      ! kg
            nwfa2d(i,j) = nwfa(i,j,1) * 0.000196 * (airmass*5.E-11)
            do k = 2, kte
               nwfa(i,j,k) = naCCN1+naCCN0*exp(-((hgt(i,j,k)-hgt(i,j,1))/1000.)*niCCN3)
            enddo
         enddo
         enddo
      else
      ...
      if (max_test .lt. eps) then
         ...
         do i = its, min(ide-1,ite)
            ...
            nifa(i,j,1) = naIN1+naIN0*exp(-((hgt(i,j,2)-hgt(i,j,1))/1000.)*niIN3)
            do k = 2, kte
               nifa(i,j,k) = naIN1+naIN0*exp(-((hgt(i,j,k)-hgt(i,j,1))/1000.)*niIN3)
            enddo
         enddo
         enddo
      else
...
!+---+-----------------------------------------------------------------+
      SUBROUTINE mp_gt_driver(qv, qc, qr, qi, qs, qg, ni, nr, nc,       &
                              ...
                              t3d, pii, p, w, dz, dt_in, itimestep,      &
                              RAINNC, RAINNCV, &
                              SNOWNC, SNOWNCV, &
                              ...
                              nout3d,out3d,eqtset,getvt,              &
                              vtc,vtr,vti,vts,vtg,                    &
                              ids,ide, jds,jde, kds,kde, &             ! domain dims
                              ims,ime, jms,jme, kms,kme, &             ! memory dims
                              its,ite, jts,jte, kts,kte)               ! tile dims

      use input , only : timestats,mytime,time_microphy,time_dbz
      implicit none
...
!..Local variables
      ...
      REAL:: dt, pptrain, pptsnow, pptgraul, pptice, rdt
...
!!!      INTEGER, OPTIONAL, INTENT(IN) :: ke_diag
      ...
      REAL, DIMENSION(kts:kte+1):: vtik, vtrk, vtsk, vtgk, vtck
      real :: tem1,tem2
...
!$omp t1d,p1d,w1d,dz1d,qv1d,qc1d,qi1d,qr1d,qs1d,qg1d,ni1d,nr1d,   &
!$omp rho,prv_rev,prw_vcd,tem2,dbz,nc1d,nwfa1d,nifa1d,nwfa1,vtik,vtrk,vtsk,vtgk,vtck)
      ...
      i_loop:  do i = i_start, i_end
         ...
         call mp_thompson(qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d,     &
                      ...
                      pptrain, pptsnow, pptgraul, pptice, prv_rev, prw_vcd, &
                      vtik,vtrk,vtsk,vtgk,vtck, &

                      kts, kte, dt, i, j, eqtset)
...
!!!         RAINNC(i,j) = RAINNC(i,j) + pptrain + pptsnow + pptgraul + pptice
         ...
         prate(i,j) = rdt*( pptrain + pptsnow + pptgraul + pptice )
...
!!!          ENDIF
         ...
         IF (has_reqc.ne.0 .and. has_reqi.ne.0 .and. has_reqs.ne.0) THEN
          ...
          do k = kts, kte
             re_cloud(i,j,k) = MAX(RE_QC_BG, MIN(re_qc1d(k), 50.E-6))
             re_ice(i,j,k)   = MAX(RE_QI_BG, MIN(re_qi1d(k), 125.E-6))
             re_snow(i,j,k)  = MAX(RE_QS_BG, MIN(re_qs1d(k), 999.E-6))
          enddo
         ENDIF
...
!..Local variables
      ...
      DOUBLE PRECISION, DIMENSION(kts:kte), intent(inout) :: prw_vcd
      ...
      DOUBLE PRECISION, DIMENSION(kts:kte), intent(inout) :: prv_rev
      ...
      ! GHB: get fall velocities
!!!      REAL, DIMENSION(kts:kte+1):: vtik, vtnik, vtrk, vtnrk, vtsk, vtgk,  &
!!!           vtck, vtnck
      REAL, DIMENSION(kts:kte+1):: vtnik, vtnrk, vtnck
      REAL, DIMENSION(kts:kte+1),intent(inout) :: vtik,vtrk,vtsk,vtgk,vtck
...
!+---+-----------------------------------------------------------------+
      do k = kts, kte
         ...
         if(eqtset.eq.2)then
           rcvm(k) = 1.0/( cv+cvv*qv1d(k)+cpl*(qc1d(k)+qr1d(k))   &
...
!+---+-----------------------------------------------------------------+
          ...
          elseif(eqtset.eq.2)then
          econd = (lvap(k)-Rv*temp(k))*rcvm(k)
          tten(k) = tten(k) + econd*prw_vcd(k)*(1-IFDRY)
          endif
