...
MODULE MODULE_MP_MORR_TWO_MOMENT2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE MORR_TWO_MOMENT_INIT2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
END SUBROUTINE MORR_TWO_MOMENT_INIT2
...
SUBROUTINE MP_MORR_TWO_MOMENT2(ITIMESTEP,                       &
...
! add cumulus tendencies, decouple from mu
          qrcu1d(k)     = 0!qrcuten(i,k,j)/mu(i,j)
          qscu1d(k)     = 0!qscuten(i,k,j)/mu(i,j)
          qicu1d(k)     = 0!qicuten(i,k,j)/mu(i,j)
      end do

      call MORR_TWO_MOMENT_MICRO2(QC_TEND1D, QI_TEND1D, QNI_TEND1D, QR_TEND1D,            &
           NI_TEND1D, NS_TEND1D, NR_TEND1D,                                                  &
...
END SUBROUTINE MP_MORR_TWO_MOMENT2

!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
SUBROUTINE MORR_TWO_MOMENT_MICRO2(QC3DTEN,QI3DTEN,QNI3DTEN,QR3DTEN,         &
...
!                to absolute tendencies need to be recalculated at the
       ! end
      REAL, DIMENSION(KTS:KTE) ::  QC3D_IN            ! CLOUD WATER MIXING RATIO (KG/KG)
      REAL, DIMENSION(KTS:KTE) ::  QI3D_IN            ! CLOUD ICE MIXING RATIO (KG/KG)
      REAL, DIMENSION(KTS:KTE) ::  QNI3D_IN           ! SNOW MIXING RATIO (KG/KG)
      REAL, DIMENSION(KTS:KTE) ::  QR3D_IN            ! RAIN MIXING RATIO (KG/KG)
      REAL, DIMENSION(KTS:KTE) ::  NI3D_IN            ! CLOUD ICE NUMBER CONCENTRATION (1/KG)
      REAL, DIMENSION(KTS:KTE) ::  NS3D_IN            ! SNOW NUMBER CONCENTRATION (1/KG)
...
!KiD B.Shipway - input values can be modified before tendencies are calculated
!                to absolute tendencies need to be recalculated at the end
      !kte=nz
      !print*, kts, kte
...
! AIR DENSITY
            ...
            !print*, rho(k), pres(k), qvs(k),qvi(k)
...
!          END IF
      !KiD
      ! BJS: Note that I've moved these inside the k loop
      ! because they are not always initialised outside of cloud
      if ( nx == 1) then 
...
!KiD B.Shipway - input values can be modified before tendencies are calculated
!                to absolute tendencies need to be recalculated at the end
      QC3DTEN = (QC3D - QC3D_IN)/DT
      QI3DTEN = (QI3D - QI3D_IN)/DT
      QNI3DTEN = (QNI3D - QNI3D_IN)/DT
      QR3DTEN = (QR3D - QR3D_IN)/DT
      NI3DTEN = (NI3D - NI3D_IN)/DT
      NS3DTEN = (NS3D - NS3D_IN)/DT
      NR3DTEN = (NR3D - NR3D_IN)/DT
      QG3DTEN = (QG3D - QG3D_IN)/DT
      NG3DTEN = (NG3D - NG3D_IN)/DT

      !print*, 'all done'
! ALL DONE !!!!!!!!!!!
      RETURN
    END SUBROUTINE MORR_TWO_MOMENT_MICRO2
...
!+---+-----------------------------------------------------------------+
  ...
  SUBROUTINE getqvs(T3D,QV3D,QVS,PRES,XXLS,XXLV,CPM,KTS,KTE)
  ...
  SUBROUTINE SIMPLE_MICRO3_2(QC3D,QI3D,QS3D,QR3D, &
            T3D,QV3D,PRES,DZQ,   &
            DT,                                                   &
            KTS,KTE,QG3D)
       ...
       ! INPUT/OUTPUT PARAMETERS                                 ! DESCRIPTION (UNITS)
             ...
             REAL, DIMENSION(KTS:KTE) ::  QC3DTEN            ! CLOUD WATER MIXING RATIO TENDENCY (KG/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  QI3DTEN            ! CLOUD ICE MIXING RATIO TENDENCY (KG/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  QS3DTEN           ! SNOW MIXING RATIO TENDENCY (KG/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  QR3DTEN            ! RAIN MIXING RATIO TENDENCY (KG/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  NI3DTEN            ! CLOUD ICE NUMBER CONCENTRATION (1/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  NS3DTEN            ! SNOW NUMBER CONCENTRATION (1/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  NR3DTEN            ! RAIN NUMBER CONCENTRATION (1/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  QC3D               ! CLOUD WATER MIXING RATIO (KG/KG)
             REAL, DIMENSION(KTS:KTE) ::  QI3D               ! CLOUD ICE MIXING RATIO (KG/KG)
             REAL, DIMENSION(KTS:KTE) ::  QS3D              ! SNOW MIXING RATIO (KG/KG)
             REAL, DIMENSION(KTS:KTE) ::  QR3D               ! RAIN MIXING RATIO (KG/KG)
             REAL, DIMENSION(KTS:KTE) ::  NI3D               ! CLOUD ICE NUMBER CONCENTRATION (1/KG)
             REAL, DIMENSION(KTS:KTE) ::  NS3D               ! SNOW NUMBER CONCENTRATION (1/KG)
             REAL, DIMENSION(KTS:KTE) ::  NR3D               ! RAIN NUMBER CONCENTRATION (1/KG)
       ...
       ! HM, ADD 1/16/07, SEDIMENTATION TENDENCIES FOR MIXING RATIO
             ...
             REAL, DIMENSION(KTS:KTE) ::  QRSTEN            ! RAIN SED TEND (KG/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  QISTEN            ! CLOUD ICE SED TEND (KG/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  QSSTEN           ! SNOW SED TEND (KG/KG/S)
             REAL, DIMENSION(KTS:KTE) ::  QCSTEN            ! CLOUD WAT SED TEND (KG/KG/S)      
       ...
       ! FALL SPEED WORKING VARIABLES (DEFINED IN CODE)
             ...
             REAL FALTNDNR
             REAL, DIMENSION(KTS:KTE) ::   FNR
             integer, dimension(kts:kte) :: ikflag
       ...
       !                to absolute tendencies need to be recalculated at the
             ...
             REAL, DIMENSION(KTS:KTE) ::  QC3D_IN            ! CLOUD WATER MIXING RATIO (KG/KG)
             REAL, DIMENSION(KTS:KTE) ::  QI3D_IN            ! CLOUD ICE MIXING RATIO (KG/KG)
             REAL, DIMENSION(KTS:KTE) ::  QS3D_IN           ! SNOW MIXING RATIO (KG/KG)
             REAL, DIMENSION(KTS:KTE) ::  QR3D_IN            ! RAIN MIXING RATIO (KG/KG)
             REAL, DIMENSION(KTS:KTE) ::  NI3D_IN            ! CLOUD ICE NUMBER CONCENTRATION (1/KG)
             REAL, DIMENSION(KTS:KTE) ::  NS3D_IN            ! SNOW NUMBER CONCENTRATION (1/KG)
             REAL, DIMENSION(KTS:KTE) ::  NR3D_IN            ! RAIN NUMBER CONCENTRATION (1/KG)
             ...
             real :: xinput(100,10), yinput(100,5)
             integer :: nmpoints 
             integer :: ii,ij,itile,imp
       !CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
       ...
       ! MAKE SURE ICE SATURATION DOESN'T EXCEED WATER SAT. NEAR FREEZING
            ...
            do k=kts,kte
               ...
               if (qvqvsi(k).gt.1.and.t3d(k).lt.273.15) then
                  !cond2=0.5*(qv3d(k)-qvi(k))
                  !qi3d(k)=qi3d(k)+cond2
                  !qv3d(k)=qv3d(k)-cond2
                  !t3d(k)=t3d(k)+cond2*XXLs(k)/cpm(k)
               endif
               if(qc3d(k)>0.0001) then
                  acr=qc3d(k)-0.0001
                  qr3d(k)=qr3d(k)+acr
                  qc3d(k)=0.0001
               endif
               if(qi3d(k)>0.0001) then
                  acr=qi3d(k)-0.0001
                  qs3d(k)=qs3d(k)+acr
                  qi3d(k)=0.0001
               endif
            ...
            enddo
            !return
            !print*, 'snow1',vterm_snow
            !print*, 'rain1',vterm_rain
            !print*, 'rain2',zpos
            !print*, zpos_snow
            !stop
            qr_int=0
            ...
            do k=kts,kte-1
                  ...
                  if(isnan(qr_int(k))) then
                     print*, qr3d(k),qr3d(k+1),rho(k),rho(k+1),t3d(k),t3d(k+1),pres(k:k+1)
                     print*, dzq(k+1),zpos(k+1),xf1,xf2
                     stop
                  endif
                  xf1=zpos_snow(k)
            ...
            !print*, qr3d(kts:kte-1)
            !print*, qs3d(kts:kte-1)
            !print*, kts,kte
            !print*, rho
            !stop
      end subroutine SIMPLE_MICRO3_2

      subroutine autoconv_sed(kts,kte,qc3d,qr3d,qi3d,qs3d,rho,dzq,dt)
        implicit none
        integer k, kts, kte
        real :: qc3d(kts:kte), qr3d(kts:kte), qi3d(kts:kte), qs3d(kts:kte), dzq(kts:kte), &
             zpos_rain(kts:kte),rho(kts:kte), zpos_snow(kts:kte), vterm_rain(kts:kte), vterm_snow(kts:kte)
        real :: acr, xf1,xf2
        real :: qr_int(kts:kte), qs_int(kts:kte), dt, zpos(kts:kte)
        do k=kts,kte
           if(qc3d(k)>0.0001) then
              acr=qc3d(k)-0.0001
              qr3d(k)=qr3d(k)+acr
        ...
        qr3d(kts:kte-1)=qr_int(kts:kte-1)
        qs3d(kts:kte-1)=qs_int(kts:kte-1)
 end subroutine autoconv_sed

END MODULE MODULE_MP_MORR_TWO_MOMENT2
