...
!----------------------------------------------------------------------
     ...
     &                 ,P1000mb,U10E,V10E                              &
     &                 ,psim,psih,fm,fhh,z0out,usemyjpbl,rhosfc,isftcflx  &
...
!
     ...
     &                                              ,U10,V10,TH02,Q02  &
     &                                              ,U10E,V10E
      REAL,DIMENSION(IMS:IME,JMS:JME),INTENT(OUT) :: psim,psih,fm,fhh,z0out
      logical, intent(in) :: usemyjpbl
      REAL,DIMENSION(IMS:IME,JMS:JME),INTENT(IN)    :: rhosfc
...
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE) :: pblsave
!
!----------------------------------------------------------------------
!**********************************************************************
!----------------------------------------------------------------------
...
!!!    zsl:  height of lowest model level
!!!    print *,'  zsl = ',zsl
...
!
     ...
     &               ,U10E(I,J),V10E(I,J)                              &
     &               ,psim(i,j),psih(i,j),fm(i,j),fhh(i,j),z0out(i,j),rhosfc(i,j),isftcflx  &
     &               ,IDS,IDE,JDS,JDE,KDS,KDE                          &
     &               ,IMS,IME,JMS,JME,KMS,KME                          &
...
!----------------------------------------------------------------------
     ...
     &                 ,U10E,V10E                                      &
     &                 ,psim,psih,fm,fhh,z0out,rhosfc,isftcflx         &
     &                 ,IDS,IDE,JDS,JDE,KDS,KDE                        &
     &                 ,IMS,IME,JMS,JME,KMS,KME                        &
...
!
     ...
     &                   ,TH02,TH10,U10,V10,U10E,V10E
      REAL,INTENT(OUT) :: psim,psih,fm,fhh,z0out
      REAL,INTENT(IN)  :: rhosfc
      integer, optional, intent(in) :: isftcflx
!
...
!----------------------------------------------------------------------
        ...
        !----------
        ! GHB, 200621: level-off roughness length at high wind speeds:
          IF ( PRESENT(ISFTCFLX) ) THEN
             IF ( ISFTCFLX.NE.0 ) THEN
                ...
                Z0=(1.0-ZW) * ZN1 + ZW * ZN2
                Z0=MIN(Z0,2.85e-3)
!!!                Z0=MAX(Z0,1.27e-7)
                ! keep this from orig myjsfc (see above):
                Z0=MAX(Z0,1.59E-5)
             ENDIF
          ENDIF
        !----------
...
!!!      RLOW=PLOW/(R_D*TLOW)
      ...
      HFX=-RLOW*CP*HSFLX
      QFX=-RLOW*HLFLX*WETM
!!!      ! ghb, 200619: use qsfc from myjpbl:
!!!      !!! HLFLX=AKHS*(QLOW -QZ0 )
!!!      exnsfc = (1.E5/PSFC)**CAPA
!!!      qspbl = PQ0SEA/PSFC   *EXP(A2*(THS-A3*EXNSFC)/(THS-A4*EXNSFC))
!!!      QFX=-RLOW*AKHS*(QLOW - qspbl )*WETM
      FLX_LH=XLV*QFX
      FLHC=RLOW*CP*AKHS
      FLQC=RLOW*AKHS*WETM
...
!     &         *EXP(A2S*(TSK-A3S)/(TSK-A4S)) 
!!!!        QS=QS/(1.-QS)
!!!!        esl = 611.2 * EXP( 17.67 * ( TSK  - 273.15 ) / ( TSK  - 29.65 ) )
!!!!        esl = min( esl , psfc*0.5 )
!!!!        qs = 0.6219718 * ESL /(PSFC-ESL)
!      ENDIF
!----------------------------------------------------------------------
!
