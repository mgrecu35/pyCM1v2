...
! ADD SEDIMENTATION TENDENCIES
                                  QGSTEN,QRSTEN,QISTEN,QNISTEN,QCSTEN,qvs,qvi,ised)
...
! SET REALISTIC LIMITS ON FALLSPEED
      ! sedimentation velocities are set to 0.0
        UMS=ised*MIN(UMS,1.2)
        UNS=ised*MIN(UNS,1.2)
        UMI=ised*MIN(UMI,1.2)
        UNI=ised*MIN(UNI,1.2)
        UMR=ised*MIN(UMR,9.1)
        UNR=ised*MIN(UNR,9.1)
        UMG=ised*MIN(UMG,20.) 
        UNG=ised*MIN(UNG,20.)
