module module_mp_jensen_ishmael
  ...
  !..cm1
  private
  public :: jensen_ishmael_init, mp_jensen_ishmael
  ...
  !.. cm1
  real, parameter, private :: CV   = CP - RD
  real, parameter, private :: CVV  = 1408.5
  real, parameter, private :: CPL  = 4190.0
  real, parameter, private :: CPI  = 2106.0
  real, parameter, private :: CPV  = 1870.0    
  real, parameter, private :: EP_2 = RD / RV
  real, parameter, private :: XLV0 = 2501000.0
  real, parameter, private :: LV1  = XLV0+(CPL-CPV)*273.15
  real, parameter, private :: LV2  = CPL-CPV
...
contains
  ...
  subroutine jensen_ishmael_init(myid)
    implicit none
...
!!!       open(20, FILE='ishmael-qi-qc.bin', FORM='unformatted')
       open(20, FILE='ishmael-qi-qc.bin', FORM='unformatted', convert='big_endian')
       do i1 = 1,51
          do i2 = 1,51
             do i3 = 1,51
                do i4 = 1,11
       ...
       enddo
       close(20)
       if(myid.eq.0) print*, 'Jensen_ISHMAEL lookup table 1 of 3'
    endif
...
!!!       open(30, FILE='ishmael-qi-qr.bin', FORM='unformatted')
       open(30, FILE='ishmael-qi-qr.bin', FORM='unformatted', convert='big_endian')
       do i1 = 1,51
          do i2 = 1,51
             do i3 = 1,51
                do i4 = 1,11
       ...
       enddo
       close(30)
       if(myid.eq.0) print*, 'Jensen_ISHMAEL lookup table 2 of 3'
    endif
...
!!!       open(40, FILE='ishmael-gamma-tab.bin', FORM='unformatted')
       open(40, FILE='ishmael-gamma-tab.bin', FORM='unformatted', convert='big_endian')
       do i1 = 1,505001
          read(40) gamma_tab_o(i1)
       enddo
       close(40)
       if(myid.eq.0) print*, 'Jensen_ISHMAEL lookup table 3 of 3'
    endif
  ...
  !.. Call to build the aggregation lookup table (JYH)
    ...
    if(myid.eq.0) print*, 'Jensen_ISHMAEL aggregation lookup table built'
  ...
  subroutine  MP_JENSEN_ISHMAEL(ITIMESTEP, DT_IN, P, DZ, &  
       T3D, QV, QC, QR, NR, QI1, NI1, AI1, CI1,          & !.. TH -> T3D for cm1
       QI2, NI2, AI2, CI2,                               &
       QI3, NI3, AI3, CI3,                               &
       ...
       ruh, rvh, rmh, rr, rain, prate,                           &
       ib3d, ie3d, jb3d, je3d, kb3d, ke3d, eqtset,               &
       nout3d, out3d, dowr, outfile, getdbz, dorad )
  ...
  !.. cm1
    use input, only : timestats, mytime, time_microphy, time_dbz,ibr,ier,jbr,jer,kbr,ker
    implicit none
  ...
  !.. 3D variables
    real, dimension(ims:ime, jms:jme, kms:kme), intent(in) :: p, dz

    real, dimension(ims:ime, jms:jme, kms:kme), intent(inout) ::     &
         t3d, qv, qc, qr, nr, qi1, ni1, ai1, ci1, qi2, ni2, ai2, ci2,&
         qi3, ni3, ai3, ci3

    real, dimension(ibr:ier,jbr:jer,kbr:ker), optional, intent(inout) ::     &
         diag_effc3d, diag_effi3d

    real, dimension(ims:ime, jms:jme, kms:kme), optional, intent(inout) ::     &
         diag_dbz3d

    ! note size of p3 arrays for diag output:
    real, dimension(its:ite, jts:jte, kts:kte), optional, intent(inout) ::     &
         diag_vmi3d_1, diag_di3d_1, diag_rhopo3d_1, diag_phii3d_1,   &
         diag_vmi3d_2, diag_di3d_2, diag_rhopo3d_2, diag_phii3d_2,   &
         diag_vmi3d_3, diag_di3d_3, diag_rhopo3d_3, diag_phii3d_3,   &
         diag_itype_1, diag_itype_2, diag_itype_3                                    
  ...
  !.. 2D variables
    ...
    logical, intent(in) :: dowr,getdbz,dorad
  ...
  !.. cm1
    ...
    REAL, DIMENSION(ims:ime, jms:jme, nrain), INTENT(INOUT) :: rain
    REAL, DIMENSION(ims:ime, jms:jme),        INTENT(INOUT) :: prate
    ...
    real :: rdt
    integer :: n
    DOUBLE PRECISION, DIMENSION(kts:kte):: prv_rev,prw_vcd
    real :: tem1,tem2
    double precision, dimension(jds:jde) :: bud1,bud2,bud3,bud4
  ...
  !.. Thermo,rain,cloud,vapor,etc
             TH1D(k)         =  T3D(i,j,k)*(100000./P(i,j,k))**(RCP) !.. convert to theta for micro
             QV1D(k)         =  QV(i,j,k)
             QC1D(k)         =  QC(i,j,k)
             QR1D(k)         =  QR(i,j,k)
             NR1D(k)         =  NR(i,j,k)
             ...
             DBZ1D(k)        =  -35.
  !.. Planar-nucleated
             QI1D(ICE1,k)    = QI1(i,j,k)
             NI1D(ICE1,k)    = NI1(i,j,k)
             AI1D(ICE1,k)    = AI1(i,j,k)
             CI1D(ICE1,k)    = CI1(i,j,k)
 !            VMI1D(ICE1,k)   = 0.
 !            DI1D(ICE1,k)    = 0.
 !            RHOPO1D(ICE1,k) = 0.
 !            PHII1D(ICE1,k)  = 0.
 !            ICETYPE1D(ICE1,k) =  0.
  !.. Columnar-nucleated
             QI1D(ICE2,k)    = QI2(i,j,k)
             NI1D(ICE2,k)    = NI2(i,j,k)
             AI1D(ICE2,k)    = AI2(i,j,k)
             CI1D(ICE2,k)    = CI2(i,j,k)
 !            VMI1D(ICE2,k)   = 0.
 !            DI1D(ICE2,k)    = 0.
 !            RHOPO1D(ICE2,k) = 0.
 !            PHII1D(ICE2,k)  = 0.
 !            ICETYPE1D(ICE2,k) =  0.
  !.. Aggregates
             QI1D(ICE3,k)    = QI3(i,j,k)
             NI1D(ICE3,k)    = NI3(i,j,k)
             AI1D(ICE3,k)    = AI3(i,j,k)
             CI1D(ICE3,k)    = CI3(i,j,k)
 !            VMI1D(ICE3,k)   = 0.
 !            DI1D(ICE3,k)    = 0.
 !            RHOPO1D(ICE3,k) = 0.
 !            PHII1D(ICE3,k)  = 0.
 !            ICETYPE1D(ICE3,k) =  0.
  ...
  !.. cm1
               prv_rev, prw_vcd, eqtset,dowr,outfile)

  !.. Send the 1D columns from newmicro back to WRF
          do k=kts,kte !.. k loop (vertical)
             T3D(i,j,k)    =         TH1D(k)/(100000./P(i,j,k))**(RCP) !.. convert to temp
             QV(i,j,k)     =         QV1D(k)       
             QC(i,j,k)     =         QC1D(k)
             QR(i,j,k)     =         QR1D(k)
             NR(i,j,k)     =         NR1D(k) 
  !.. Planar-nucleated
             QI1(i,j,k)    =    QI1D(ICE1,k) 
             NI1(i,j,k)    =    NI1D(ICE1,k) 
             AI1(i,j,k)    =    AI1D(ICE1,k) 
             CI1(i,j,k)    =    CI1D(ICE1,k)  
             DIAG_VMI3D_1(i,j,k)  = vmi1d(ICE1,k)
             DIAG_DI3D_1(i,j,k)   = di1d(ICE1,k)
             DIAG_RHOPO3D_1(i,j,k)= rhopo1d(ICE1,k)
             DIAG_PHII3D_1(i,j,k) = phii1d(ICE1,k)
!             DIAG_ITYPE_1(i,j,k)  = icetype1d(ICE1,k)
  !.. Columnar-nucleated
             QI2(i,j,k)    =    QI1D(ICE2,k) 
             NI2(i,j,k)    =    NI1D(ICE2,k) 
             AI2(i,j,k)    =    AI1D(ICE2,k) 
             CI2(i,j,k)    =    CI1D(ICE2,k)  
             DIAG_VMI3D_2(i,j,k)  = vmi1d(ICE2,k)
             DIAG_DI3D_2(i,j,k)   = di1d(ICE2,k)
             DIAG_RHOPO3D_2(i,j,k)= rhopo1d(ICE2,k)
             DIAG_PHII3D_2(i,j,k) = phii1d(ICE2,k)
!             DIAG_ITYPE_2(i,j,k)  = icetype1d(ICE2,k)
  !.. Aggregates
             QI3(i,j,k)    =    QI1D(ICE3,k) 
             NI3(i,j,k)    =    NI1D(ICE3,k) 
             AI3(i,j,k)    =    AI1D(ICE3,k) 
             CI3(i,j,k)    =    CI1D(ICE3,k)  
             DIAG_VMI3D_3(i,j,k)  = vmi1d(ICE3,k)
             DIAG_DI3D_3(i,j,k)   = di1d(ICE3,k)
             DIAG_RHOPO3D_3(i,j,k)= rhopo1d(ICE3,k)
             DIAG_PHII3D_3(i,j,k) = phii1d(ICE3,k)
!             DIAG_ITYPE_3(i,j,k)  = icetype1d(ICE3,k)
          enddo

        IF( dorad )THEN
          do k=kts,kte
             DIAG_EFFC3D(i,j,k)  =   max((min(EFFC1D(k),50.e-6)),2.51e-6)
             DIAG_EFFI3D(i,j,k)  =   max((min(EFFI1D(k),999.e-6)),5.e-6)
          enddo
        ENDIF
        IF( getdbz )THEN
          do k=kts,kte
             DIAG_DBZ3D(i,j,k)   =   max(dbz1d(k),-35.)
          enddo
        ENDIF
  ...
  !.. cm1
          do n = 1,nrain
             RAIN(i,j,n) = RAIN(i,j,n) + 0.1*( qrpre1d + qipre1d(ICE1) + qipre1d(ICE2) + qipre1d(ICE3) )
             !.. mult by 100 cm/m  (convert from m to cm)
             !.. div by 1000 kg/m^3  (density of water)
          enddo
          
          prate(i,j) = rdt*( qrpre1d + qipre1d(ICE1) + qipre1d(ICE2) + qipre1d(ICE3) )
          
          bud1(j) = bud1(j) + ( qrpre1d + qipre1d(ICE1) + qipre1d(ICE2) + qipre1d(ICE3) )*ruh(i)*rvh(j)*dx*dy

          do k = kts, kte
             tem2 = dt_in*rr(i,j,k)*ruh(i)*rvh(j)*rmh(i,j,k)*tem1
             bud2(j) = bud2(j) + tem2*prv_rev(k)
             bud3(j) = bud3(j) + tem2*max(prw_vcd(k),0.0d0)
             bud4(j) = bud4(j) - tem2*min(prw_vcd(k),0.0d0)
          enddo
          
          ! Ice-one precipitation
...
!!          RAINNCV(i,j) = qrpre1d + qipre1d(ICE1) + qipre1d(ICE2) + qipre1d(ICE3)
  ...
  !.. cm1
    integer, intent(in) :: eqtset
    double precision, dimension(kts:kte), intent(inout) :: prw_vcd, prv_rev
    real, dimension(kts:kte) :: rcvm
  ...
  !.. Some dummy variables
    ...
    real :: tmpsum, tmpsumr, tmpmelt, savet, saveqc
  ...
  !.. cm1
       if(eqtset.eq.2)then
          rcvm(k) = 1.0/( cv+cvv*qv(k)+cpl*(qc(k)+qr(k))   &
               +cpi*(qi(ICE1,k)+qi(ICE2,k)+qi(ICE3,k)) )
       endif
  ...
  !..Use the old density and deltastr for loss values e.g. q1 = q1 - (transfered to q2)
                ...
                gamma_arg = NU+2.+deltastr(cc)
                gi=MIN((MAX((NINT((gamma_arg*100000.)-355000+1)),1)),505001)
!!!              if(dowr) write(outfile,*) cc,k,betam,gi,gamma_tab(gi),alphv,ni(cc,k),rhobar(cc),gammnu,qi(cc,k),deltastr(cc)
                ani(cc)=((qi(cc,k)*gammnu)/(rhobar(cc)*ni(cc,k)*alphv* &
                     gamma_tab(gi)))**(1./betam)
                cni(cc)=ao**(1.-deltastr(cc))*ani(cc)**deltastr(cc)
